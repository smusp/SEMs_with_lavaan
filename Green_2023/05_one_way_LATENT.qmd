...
The model path diagram for the one-way ANOVA of a latent variable is shown in Fig 21.3 (p. 403), and is reproduced below. The diagram shows the "Less Constrained" model.
...
Also for the purposes of identification and scaling, the latent mean for the first group is constrained to zero. For the "Less Constrained" model, the latent means for the other groups (a~2~ and a~3~) are freely estimated.
...
- TLG impose one last constraint - latent variances are constrained to equality across the groups (they do this to obtain a pooled variance to calculate an effect size for the differences between latent means).

(Note: TLG refer to "disturbances" and "error variances" for the latent variable, and their path diagram shows a disturbance for the latent. I don't know that this is correct.)
...
One could scroll through the model summaries to find the latent means and variances for the "Less Constrained" model, and compare them with Table 21.6. They are also needed to calculate effect sizes (given in the first column on page 405).
...
But it is probably safer to extract latent means and variances from a list of parameter estimates, then substitute into the formula for effect size.
...
Compare the effect sizes with those given on page 405, and the means and variances with those in Table 21.6.
...
Note that the residual variances are freely estimated across groups, as are the latent variances. The indicator covariances are by default set to zero (unless there is good reason to have one or more estimated).
...
#### Model that applies to the 2nd row in Table 21.6: "One measure - Y~1~"
...
Fit the models and get the latent means, latent variances, and the $\upchi$^2^ test.
...
Compare with the latent means and variances, and the $\upchi$^2^ test in the 2nd row in Table 21.6.
...
I need sample means and covariances - they can be extract from a list of sample statistics. I need estimated indicator intercepts and residual variances - they can be extracted from `estimates`. Also I need estimated latent means and variances - they have already been extracted.
...
- Differences between "y1" variances and "y1" residual variances are the latent variances
...
#### Model that applies to the 3rd row in Table 21.6: "One measure - Y~2~"
...
Compare with the latent means and variances, and the $\upchi$^2^ test in the 3rd row in Table 21.6.
...
- Latent variances already extracted in `LatentVar`
- Differences between "y2" variances and "y2" residual variances are the latent variances
