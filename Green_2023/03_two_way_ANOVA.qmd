...
The model statements are shown below. The "Less Constrained" model allows the means (represented by the labels, am, af, ..., cf) to differ across the groups. The constraints statements are added to the "Less Constrained" statement to give the "More Constrained" models. The "More Constrained" models are constrasted with the "Less Constrained" model to test for the Gender and Coping Strategy main effects (weighted and unweighted) and the Gender X Coping Strategy interaction. In each case the residual variances are constrained to equality.
