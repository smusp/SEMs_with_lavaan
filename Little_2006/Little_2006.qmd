...
LS&C discuss methods for identification and scaling of latent variables in SEMs. These methods are:  
...
Use the `getCov()` function from the **lavaan** package to obtain the full variance/covariance matrix for each group (using the `lapply()` function to return the two (co)variance matrices in a list).
