...
Use the `lav_getcov()` function from the **lavaan** package to obtain the full co/variance matrix for each group (using the `Map()` function to apply the `lav_getcov()` function to the lists, and to return the two co/variance matrices in a list).
