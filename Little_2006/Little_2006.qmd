...
Little et al present effects-coding method for identification and scaling of latent variables in SEMs. They also present traditional methods: marker-variable method and reference-group method. 

  - Reference Group Method - For each construct, latent variance fixed to one and latent mean fixed to zero; constrain the mean and variance in the first group only. With invariance of loadings and intercepts, latent means and variances are freely estimated in subsequent groups.
  - Marker Variable Method - For each construct, loading of one indicator fixed to one and intercept for chosen indicator fixed to zero; constrain these loadings and intercepts in both groups.
  - Effects Coding method - For each construct, contraint loading to add to the number of indicators, and indicator intercepts to add to zero.

They present a two-group (7th grade and 8th grade), two-construct (positive affect and negative affact) model. Each construct is assessed with three manifest indicators. The SEM diagram below shows the model as it might apply to the whole sample. The purpose is to explain the symbols used by Little et al, and to show how they apply in the diagram. First, POS and NEG are the constructs; and pos~1~, ..., neg~3~ are the indicators. The solid lines represent the covariance structure, and the gray lines represent the mean structure (ie, the means and intercepts). 
...
Use the `getCov()` function from the lavaan package to obtain the covariance matrix for each group.
...
Second, Little et. al. state that the data display strong metric invariance (p. 63); that is, the corresponding loadings and intercepts are equal across the groups. There is no need to be concerned with these constraints when constructing the model statement - they will be set up in the next step. (Strictly, the intercepts do even need to be mentioned in the model statement - lavaan will add them automatically when `sample.means` are in the model. But they are left in the statement below because intercepts are implicated in constraints in models to follow.)
...
Finally, lavaan's default referent-loading scaling method has to be explicitely disabled, by pre-multiplying the first indicator be `NA`. 
...
Lavaan can do referent-group scaling automatically -  set `std.lv = TRUE` in the `sem()` function. The constraints are the same as above - latent variances are constrained to one in first group, and latent means are constrained to zero in first group.
...
Finally, lavaan's default referent-loading scaling method has to be explicitely disabled, for the POS construct by pre-multiplying the first indicator be `NA`. 
...
This is not lavaan's defaul method of scaling. The default referent scaling in lavaan constrains the loading for first indicator to one for both constructs and, because of the strong metric invariance, in both groups. So far, the default is the same the method outlined in Little et al. However, when there is a mean structure in the model, lavaan sets the latent means to zero (in the first group only). In the `summary()` function set `remove.unused = FALSE` to see the latent means.
...
Same points as before: lavaan will add indicator variances automatically; Constraints concerning strong metric invariance will be attended to in the next step; and
the default referent-loading scaling method has to be explicitely disabled.
...
The models "produce overall model fit statistics that are identical" (p. 66). The following extracts fit measures for all models presented here, and presents them in a table.
